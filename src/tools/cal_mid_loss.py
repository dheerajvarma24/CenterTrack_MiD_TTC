...
class CalMIDLoss:
    ...
    # Match gt data with pred data based on frameid and trackid.
    def match_pred_with_gt(self):
        for gt_key in self.gt_data.keys():
            gt_frameid = gt_key.split('_')[0] # gt_key consists of 'frameid_trackid'
            current_score = 0

            for pred_key in self.pred_data.keys():
                # get all the pred values in the same frame as gt.
                if pred_key.startswith(gt_frameid) and self.pred_data[pred_key][-1].lower() == self.gt_data[gt_key][-1].lower(): # check for class id also 
                    ...
                    if iou_score < 0.5:
                        continue # ignore the object with iou < 0.5
    ...
    def cal_MID_Loss(self):
        ...
        for matched_key in self.matched_data.keys():
            ...
            if float(gt_dep_ratio) > 1 : # or float(pred_dep_ratio) > 1:
                continue
...
if __name__ == '__main__':
    ...
    # hard coded path
    #gt_val_file_path = './eval_kitti_track/data/tracking/label_02_val_MiD/'
    #pred_val_file_path = '../../exp/tracking,ddd/kitti_half_MiD_test_EXP7/results_kitti_tracking/'
    ...
    print("###### time for MiD Loss calculation on val set [4,11] #########  ", time.time() - t0)
